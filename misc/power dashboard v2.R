...
sample.size=function(alpha,delta,sd,power) {
	
	n=(sd*(qnorm(1-alpha/2)+qnorm(power))/(mu-mu0))^2 
	print(n)
	}

sample.size(alpha=0.05,delta=0.5,sd=1,power=0.8)

power=function(alpha,delta,sd,n) {
	z=delta/sd*sqrt(n)
	pow=pnorm(z-qnorm(1-alpha/2))+pnorm(-z-qnorm(1-alpha/2))
	print(pow)
	}
	
power(alpha=0.05,delta=0.5,sd=1,n=32)

detect.delta=function(alpha,sd,n,power) {
	# Note: ignores the smaller tail (typical)
	dd2=(qnorm(1-alpha/2)-qnorm(1-power))^2*(sd^2)/n
	print(sqrt(dd2))
	}

detect.delta(alpha=0.05,sd=1,n=32,power=0.8)

######## Third graph
points at zero and delta
width of interval is mow=qnorm(1-alpha/2)*sd/sqrt(n)
