...
Begin VB.Form frmLR2DetAltGraph 
   Caption         =   "Linear Regression 2 Detectable Alternative:  Graphics Window"
   ClientHeight    =   8070
   ClientLeft      =   5445
   ClientTop       =   3825
   ClientWidth     =   8805
   ...
   Begin VtChartLib.VtChart VtChartDetAlt 
      ...
      _ExtentX        =   15055
      _ExtentY        =   6588
      _0              =   $"frmLR2DetAltGraph.frx":0000
      _1              =   $"frmLR2DetAltGraph.frx":0409
      _2              =   $"frmLR2DetAltGraph.frx":0812
      _3              =   $"frmLR2DetAltGraph.frx":0C1B
      _4              =   $"frmLR2DetAltGraph.frx":1024
      _5              =   $"frmLR2DetAltGraph.frx":142D
      _6              =   $"frmLR2DetAltGraph.frx":1836
      _7              =   $"frmLR2DetAltGraph.frx":1C3F
      _8              =   $"frmLR2DetAltGraph.frx":2049
      _9              =   $"frmLR2DetAltGraph.frx":2453
      _10             =   $"frmLR2DetAltGraph.frx":285D
      _11             =   $"frmLR2DetAltGraph.frx":2C66
      _12             =   $"frmLR2DetAltGraph.frx":306F
      _13             =   $"frmLR2DetAltGraph.frx":3478
      _count          =   14
      _ver            =   2
   End
   ...
   Begin VB.Label lblgraph 
      Caption         =   "Detectable difference graphs for linear regression (2 treatments)"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmLR2DetAltGraph"
...
'    VtChartDetAlt.Footnote.Text = "xbar_0=" + Format(XBAR(0)) + " xbar_1=" + Format(XBAR(1)) + " sx_0=" + Format(SX(0)) + " sx_1=" + Format(SX(1)) + " alpha=" + Format(ALPHA) + " m=" + Format(M) + " sigma=" + Format(SIGMA)
'    VtChartDetAlt.Footnote.Font.Size = 10
'    VtChartDetAlt.Footnote.Text = "alpha=" + Format(ALPHA) + " power=" + Format(numPower) + " n=" + Format(N) + " m=" + Format(M) + " sigma=" + Format(SIGMA) + " d_lambda=" + Format(D_LAMBDA) + " d_gamma=" + Format(D_GAMMA) + " sx(0)=" + Format(SX(0)) + " sx(1)=" + Format(SX(1)) + " xbar(0)=" + Format(XBAR(0)) + " xbar(1)=" + Format(XBAR(1))
    ...
    End If
'
'  The X Axis will be either slope difference or intercept difference.
'  Set the label to say which.
'
    With VtChartDetAlt
        If SLOPES = 1 Then
    ...
    End With
'
'   Check x-axis (detectable difference) range values.
'
    ...
    End If
'
'  Call apprporiate routine depending on whether the Y axis is
...
'
    ...
    Call write_graph_log(Me, VtChartDetAlt, "alpha=" + Format(ALPHA) + " power=" + _
    Format(numPower) + " n=" + Format(N) + " m=" + Format(M) + " sigma=" + _
    Format(SIGMA) + " d_lambda=" + Format(D_LAMBDA) + " d_gamma=" + _
    Format(D_GAMMA) + " sx(0)=" + Format(SX(0)) + " sx(1)=" + Format(SX(1)) + _
    " xbar(0)=" + Format(XBAR(0)) + " xbar(1)=" + Format(XBAR(1)) + _
                    vbCrLf + _
...
'        plotAtValue.Text = frmMain.LR2N.Text
...
'
    Me.lblgraph(0).WhatsThisHelpID = 366
...
'
'  Determine what diffrence is show on the X axis, slopes or intercepts.
'  Set the X-axis label accordingly.
'
    If frmMain.cboLR2CompareWhat.Text = "Intercepts" Then
        SLOPES = 2
        VtChartDetAlt.Plot.Axis(VtChAxisIdX, 1).AxisTitle.Text = "Detectable difference in intercepts"
        lblgraph(2).Caption = "X axis range (intercept difference)"
    ElseIf frmMain.cboLR2CompareWhat.Text = "Slopes" Then
        SLOPES = 1
        VtChartDetAlt.Plot.Axis(VtChAxisIdX, 1).AxisTitle.Text = "Detectable difference in regression slopes"
        lblgraph(2).Caption = "X axis range (slope difference)"
    End If
'
...
'        .ColumnLabel = "Power = " + Format(numPlotAt)
        ...
        For diffValue = numxLow To numxHigh Step STEP
            ...
            Else
                If SLOPES = 1 Then
                    yvalue = LSAMPLESIZE(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, numPower, lambda_c, SIGMA, diffValue, M, D_GAMMA)
                ElseIf SLOPES = 2 Then
                    yvalue = LSAMPLESIZE(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, numPower, lambda_c, SIGMA, D_LAMBDA, M, diffValue)
                End If
            End If
'
'   A negative return value indicates and error.  If that is the case
...
'        .ColumnLabel = "Sample size = " + Format(numPlotAt)
        ...
        For diffValue = numxLow To numxHigh Step STEP
            ...
            Else
                If SLOPES = 1 Then
                    yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, lambda_c, SIGMA, N, diffValue, M, D_GAMMA)
                ElseIf SLOPES = 2 Then
                    yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, lambda_c, SIGMA, N, D_LAMBDA, M, diffValue)
                End If
            End If
'
'   A negative return value indicates an error.  If that is the case then
'   ignore the point.
'
            If yvalue > 0 Then
'
...
'    Call CheckNumeric(frmLR2DetAltGraph.plotAtValue)
'End Sub

Private Sub xHigh_LostFocus()
    Call CheckNumeric(frmLR2DetAltGraph.xHigh)
End Sub

Private Sub xLow_LostFocus()
    Call CheckNumeric(frmLR2DetAltGraph.xLow)
End Sub

Private Sub yHigh_LostFocus()
    Call CheckNumeric(frmLR2DetAltGraph.yHigh)
End Sub

Private Sub yLow_LostFocus()
    Call CheckNumeric(frmLR2DetAltGraph.yLow)
End Sub
