...
Begin VB.Form frmLR2PowerGraph 
   Caption         =   "Linear Regression 2 Power:  Graphics Window"
   ClientHeight    =   8070
   ClientLeft      =   5445
   ClientTop       =   3825
   ClientWidth     =   8805
   ...
   Begin VtChartLib.VtChart VtChartPower 
      ...
      _ExtentX        =   15055
      _ExtentY        =   6588
      _0              =   $"frmLR2PowerGraph.frx":0000
      _1              =   $"frmLR2PowerGraph.frx":040A
      _2              =   $"frmLR2PowerGraph.frx":0813
      _3              =   $"frmLR2PowerGraph.frx":0C1C
      _4              =   $"frmLR2PowerGraph.frx":1025
      _5              =   $"frmLR2PowerGraph.frx":142E
      _6              =   $"frmLR2PowerGraph.frx":1837
      _7              =   $"frmLR2PowerGraph.frx":1C40
      _8              =   $"frmLR2PowerGraph.frx":2049
      _9              =   $"frmLR2PowerGraph.frx":2452
      _10             =   $"frmLR2PowerGraph.frx":285C
      _11             =   $"frmLR2PowerGraph.frx":2C65
      _12             =   $"frmLR2PowerGraph.frx":306E
      _13             =   $"frmLR2PowerGraph.frx":3477
      _count          =   14
      _ver            =   2
   End
   ...
   Begin VB.Label lblgraph 
      Caption         =   "Power graphs for linear regression (2 treatments)"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmLR2PowerGraph"
...
'
'  Produce the plot - SS or DETDIFF
'
...
'    If SLOPES = 1 Then
'        VtChartPower.Footnote.Text = "SD ctrl=" + Format(SX(0)) + " SD exp=" + Format(SX(1)) + " alpha=" + Format(ALPHA) + " m=" + Format(M) + " sigma=" + Format(SIGMA)
'    ElseIf SLOPES = 2 Then
'        VtChartPower.Footnote.Text = "mean ctrl=" + Format(XBAR(0)) + " mean exp=" + Format(XBAR(1)) + " SD ctrl=" + Format(SX(0)) + " SD exp=" + Format(SX(1)) + " alpha=" + Format(ALPHA) + " m=" + Format(M) + " sigma=" + Format(SIGMA)
'    End If
'    VtChartPower.Footnote.Font.Size = 10
'    VtChartPower.Footnote.Text = "alpha=" + Format(ALPHA) + " power=" + Format(numPower) + " n=" + Format(N) + " m=" + Format(M) + " sigma=" + Format(SIGMA) + " d_lambda=" + Format(D_LAMBDA) + " d_gamma=" + Format(D_GAMMA) + " sx(0)=" + Format(SX(0)) + " sx(1)=" + Format(SX(1)) + " xbar(0)=" + Format(XBAR(0)) + " xbar(1)=" + Format(XBAR(1))
...
'
    ...
    End If
'
'   Call appropriate routine to produce the plot.  X axis is detectable
'   difference or sample size.
'
    If cboXaxisContent.Text = DETDIFF Then
'
'   Check x axis (det diff) range.
'
...
'
    ...
    ElseIf cboXaxisContent.Text = ss Then
'
'   Check x axis (sample size) range.
'
        If numxLow < 0 Or Not (numxLow < numxHigh) Then
...
'
    ...
    Format(SIGMA) + " d_lambda=" + Format(D_LAMBDA) + " d_gamma=" + Format(D_GAMMA) + _
    " sx(0)=" + Format(SX(0)) + " sx(1)=" + Format(SX(1)) + _
...
'    'SeriesCount.Text = Format(numSeriesCount)
    ...
    If cboXaxisContent.Text = ss Then
        With VtChartPower
            .Plot.Axis(VtChAxisIdX, 1).AxisTitle.Text = ss
        End With
        xLow.Text = "0"
        xHigh.Text = Empty
'        If SLOPES = 1 Then
...
'        End If
    ...
    Else  'the X axis is slope difference or intercept difference
        With VtChartPower
            If SLOPES = 1 Then
...
'
    Me.lblgraph(0).WhatsThisHelpID = 368
...
'        .ColumnLabel = "Sample size = " + Format(numPlotAt)
        ...
        For diffValue = numxLow To numxHigh Step STEP
            ...
            Else
                If SLOPES = 1 Then
                    yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, dummy, SIGMA, N, diffValue, M, D_GAMMA)
                ElseIf SLOPES = 2 Then
                    yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, dummy, SIGMA, N, D_LAMBDA, M, diffValue)
                End If
            End If
'
'   A negative return value idicates an error.  Ignore the point if
'   that is the case.
'
            If yvalue > 0 Then
'
...
'        End If
        ...
        For SampleSizeValue = xLow To xHigh Step STEP
            If SLOPES = 1 Then
                yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, dummy, SIGMA, SampleSizeValue, D_LAMBDA, M, D_GAMMA)
            ElseIf SLOPES = 2 Then
                yvalue = LPOWER(numTreat, XBAR(0), SX(0), SLOPES, ALPHA, dummy, SIGMA, SampleSizeValue, D_LAMBDA, M, D_GAMMA)
            End If
...
'    Call CheckNumeric(frmLR2PowerGraph.plotAtValue)
'End Sub

Private Sub xHigh_LostFocus()
    Call CheckNumeric(frmLR2PowerGraph.xHigh)
End Sub

Private Sub xLow_LostFocus()
    Call CheckNumeric(frmLR2PowerGraph.xLow)
End Sub

Private Sub yHigh_LostFocus()
    Call CheckNumeric(frmLR2PowerGraph.yHigh)
End Sub

Private Sub yLow_LostFocus()
    Call CheckNumeric(frmLR2PowerGraph.yLow)
End Sub
